    ...
    <section className="space-y-6">
      <div className="rounded-3xl border bg-card p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Ringkasan data lintas modul untuk memantau kondisi akademik dan administrasi sekolah.
        </p>
        {!stats.dbReady && (
          <p className="mt-4 rounded-xl border border-dashed border-amber-300 bg-amber-100/60 px-4 py-3 text-sm text-amber-900">
            Database belum terhubung. Halaman tetap tampil sebagai template sistem, lalu akan otomatis aktif
            saat koneksi DATABASE_URL tersedia.
          </p>
        )}
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {cards.map((card) => (
          <Link key={card.key} href={card.href} className="rounded-2xl border bg-card p-5 shadow-sm">
